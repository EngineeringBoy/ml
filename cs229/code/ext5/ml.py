...
class Layer:
    ...
    def backward(self, grad_out):       pass

    def mpute_penalty(self):
        return 0
...
class MSELoss(Module):
    ...
    def update(y_true, y_predict, penalty=0.):
        loss = 0.5 * (np.sum(np.multiply(y_predict - y_true, y_predict - y_true)))
        loss = (loss + penalty) / len(y_true)
        loss_diff = (y_predict - y_true) / len(y_true)
